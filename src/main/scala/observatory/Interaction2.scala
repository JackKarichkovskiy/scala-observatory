...
object Interaction2 extends Interaction2Interface {
  ...
  def availableLayers: Seq[Layer] = {
    Seq(
      Layer(Temperatures, temperatureColorsPalette, 1975 to 2015),
      Layer(Deviations, deviationColorsPalette, 1991 to 2015)
    )
  }
  ...
  def yearSelection(selectedLayer: Signal[Layer], sliderValue: Signal[Year]): Signal[Year] = {
    def boundYear(range: Range, year: Year): Year = {
      if(year < range.start) range.start
      else if(year > range.end) range.end
      else year
    }

    Signal(boundYear(yearBounds(selectedLayer)(), sliderValue()))
  }
